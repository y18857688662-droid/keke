...
app.post('/deploy', (req, res) => {
  ...
  exec('cd /root/keke && git pull origin main && (grep -q client_max_body_size /etc/nginx/nginx.conf || sed -i "s/http {/http {\\n    client_max_body_size 100m;/" /etc/nginx/nginx.conf) && nginx -t && nginx -s reload && systemctl restart keke', { timeout: 30000 }, (err, stdout, stderr) => {
