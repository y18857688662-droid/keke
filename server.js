...
function toggleMic(){const btn=document.getElementById('micBtn');if(isRecording){if(recognition)recognition.stop();return}const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){alert('浏览器不支持语音');return}if(!audioCtx)audioCtx=new(window.AudioContext||window.webkitAudioContext)();if(audioCtx.state==='suspended')audioCtx.resume();const sil=audioCtx.createBufferSource();sil.buffer=audioCtx.createBuffer(1,1,22050);sil.connect(audioCtx.destination);sil.start(0);recognition=new SR();recognition.lang='zh-CN';recognition.interimResults=false;recognition.continuous=false;recognition.onstart=()=>{isRecording=true;btn.classList.add('recording');document.getElementById('status').textContent='listening…'};recognition.onresult=e=>{const text=e.results[0][0].transcript;document.getElementById('msg').value=text;chatSpeak()};recognition.onerror=()=>{isRecording=false;btn.classList.remove('recording');document.getElementById('status').textContent=''};recognition.onend=()=>{isRecording=false;btn.classList.remove('recording');if(document.getElementById('status').textContent==='listening…')document.getElementById('status').textContent=''};recognition.start()}
</script>
</body>
</html>`);
});
